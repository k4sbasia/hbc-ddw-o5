...
#############################################################################################################################
#####
#####   PROGRAM NAME : o5_channel_advisor_feed.sh
#####
...
export CTL=$HOME/CTL
export PROCESS='o5_channel_advisor_feed'
export CONTROL_FILE="$CTL/${PROCESS}.ctl"
export LOG_FILE="$LOG/${PROCESS}_log.txt"
export JOB_NAME="${PROCESS}"
export SCRIPT_NAME="${PROCESS}"
export PROCESS_SQL1='o5_channel_advisor_extract.sql'
export PROCESS_SQL2='o5_channel_advisor_export.sql'
export SFILE_SIZE=0
export FILE_NAME="$DATA/CA_o5_feed_`date +"%Y%m%d"`.txt"
export LOAD_COUNT=0
export FILE_COUNT=0
export TFILE_SIZE=0
...
sqlplus -s -l  $CONNECTDW @${SQL}/${PROCESS_SQL1} "${SCHEMA}" >>${LOG_FILE}
SQL_RET_CODE=$?
...
cd $DATA
gzip -c CA_o5_feed_`date +"%Y%m%d"`.txt > CA_o5_feed_`date +"%Y%m%d"`.txt.gz
wait
################################################################
##SQL ERROR VALIDATION
#################################################################
...
#################################################################
scp -P 22 /home/cognos/DATA/CA_o5_feed_`date +"%Y%m%d"`.txt.gz a_2954@ftp.versafeed.com:/CA_o5_feed_`date +"%Y%m%d"`.txt.gz
SCP_RET_CODE=$?
#################################################################
...
then
  echo "Finished SCP data successfully to VERSAFEED.COM at `date '+%a %b %e %T'` with return code ${SCP_RET_CODE}" >>${LOG_FILE}
else
  echo "Aborting: SCP failed  at `date '+%a %b %e %T'` with error code ${SCP_RET_CODE}" >>${LOG_FILE}
  BBODY="O5 Versa feed is delayed and we are looking into it. Thanks"
  BADDRESS='David.Mataranglo@360i.com Sofia_Azzolina@s5a.com Hannah_Tevolini@s5a.com' 
  BSUBJECT="O5 VERSA FEED DELAYED"
  ##send_delay_email
...
cd $DATA
cp CA_o5_feed_`date +"%Y%m%d"`.txt $FILE_NAME_FLIPP
TFILE_SIZE="`ls -ll $FILE_NAME_FLIPP |tr -s ' ' '|' |cut -f5 -d'|'`"
TARGET_COUNT="`wc -l $FILE_NAME_FLIPP |tr -s ' ' '|'|cut -f1 -d'|'`"
