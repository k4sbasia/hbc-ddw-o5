...
#############################################################################################################################
#####
#####   PROGRAM NAME : o5_edb_cheetah_metadata.sh
#####
#####   DESCRIPTION  : This script does the following
#####		     1. Calls the O5.P_email_metadata
#####
...
################################################################
. $HOME/params.conf o5
export PROCESS='o5_edb_cheetah_metadata'
export SQL=$HOME/SQL
export LOG=$HOME/LOG
...
set heading off
select count(*)
from O5.email_mailing_metadata_wrk wrk;
quit;
EOF`
...
exec O5.P_email_mailing_metadata;
exit;
EOF
...
set heading off
select count(*)
from O5.email_mailing_metadata
...
sqlplus -s -l  $CONNECTDW <<EOF
exec DBMS_MVIEW.REFRESH('sddw.mv_o5_email_mailing_metadata','c', ATOMIC_REFRESH => false);
show errors;
exit;
EOF
