...
#############################################################################################################################
#####
#####   PROGRAM NAME : o5_shoprunner_order_feed.sh
#####
...
export CTL=$HOME/CTL
export PROCESS='o5_shoprunner_order_feed'
export LOG_FILE="$LOG/${PROCESS}_log.txt"
export JOB_NAME="${PROCESS}"
export SCRIPT_NAME="${PROCESS}"
export FILEDATE=`date +"%Y%m%d%H%M"`
export ORDER_FEED='o5_shoprunner_order_feed.sql'

export ORDER_FEED_NAME='o5_shoprunner_order_feed.xml'
export ORDER_FILE_NAME="OFF5TH_${FILEDATE}_order-feed.xml"
...
export BBODY="Upstream commerce order feed is delayed and we are looking into it. Thanks" 
...
# sqlplus -s -l $CONNECTO5 @${SQL}/${ORDER_FEED} >>${LOG_FILE}
 sqlplus -s -l $CONNECTDWXML @${SQL}/${ORDER_FEED} >>${LOG_FILE}
 SQL_RET_CODE=$?
...
scp cognos@${ORACLESRV}:/oracle/EXPORTS/dataservices/${ORDER_FEED_NAME} ${DATA}
wait
dos2unix ${DATA}/${ORDER_FEED_NAME}>>${LOG_FILE}
echo -e "Executed the command Dos2Unix to eliminate  " >>${LOG_FILE}
mv ${DATA}/${ORDER_FEED_NAME} ${DATA}/${ORDER_FILE_NAME}
...
###############################################################
if [ `egrep -c "^ERROR|ORA-|not found|not connected"  ${ORDER_FILE_NAME}` -ne 0 ]
then        
...
then	
	TFILE_SIZE="`ls -ll  ${ORDER_FILE_NAME} |tr -s ' ' '|' |cut -f5 -d'|'`" # size of the file
	#FILE_COUNT="`wc -l  ${ORDER_FILE_NAME} |tr -s ' ' '|' |cut -f1 -d'|'`" # linecount of the file
	FILE_COUNT=`grep -o "<OrderNumber>" $ORDER_FILE_NAME |wc -l` # linecount of the file
fi
echo "File size ${ORDER_FILE_NAME} is: ${TFILE_SIZE}" >> ${LOG_FILE}
echo "Line count ${ORDER_FILE_NAME} is: ${FILE_COUNT}" >> ${LOG_FILE}
#################################################################
echo "Starting ${ORDER_FILE_NAME} SFTP process " >> ${LOG_FILE}
wait
...
#################################################################
echo -e "Shoprunner O5 Order feed Posted on `date '+%a %b %e %T'` \n\nThe File size for today's feed is : ${TFILE_SIZE} bytes\nTotal number of line count in the XML is : ${FILE_COUNT}\n\nPlease make aware of any issue to  HBC Data Management. \nThank You " |mailx -s "O5 Shoprunner Order Feed" -r O5_SHOPRUNNER_EMAIL@hbc.com Harsh_Desai@s5a.com hbcdigitaldatamanagement@saksinc.com  nsampey@shoprunner.com
#################################################################
