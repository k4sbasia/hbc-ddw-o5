...
export PROCESS='o5_linkshare_product_feed'
export LOG_FILE="$LOG/${PROCESS}_log.txt"
export JOB_NAME="${PROCESS}"
export SCRIPT_NAME="${PROCESS}"
export EXTRACT_SQL='$SQL/${PROCESS}.sql'
export EXTRACT_SQL2='$SQL/${PROCESS}_attr.sql'
...
set heading off
  select trim(count(distinct (styl_seq_num))) from O5.O5_PARTNERS_EXTRACT_WRK where wh_sellable_qty > 0;
quit;
EOF`
##########################################################################################
...
then
(echo "Off5th Linshare product feed  Counts:
UPC level count : $TARGET_COUNT
ITEM count : $LOAD_COUNT"
) |  mailx -s "Off5th Linkshare product count"  AffiliateTeamList@s5a.com hbcdigtialdatamanagement@hbc.com ls-saks@linkshare.com
fi
