...
--***********************************************************
--***********************************************************
truncate table TMP_USER_ACCOUT;
truncate table  TMP_USER_ADDRESS;
INSERT INTO TMP_USER_ACCOUT
select * from martini_store.user_account@bayprod_saks_custom t
where usa_uid is not null and t.USA_SYS_TIMESTAMP > trunc(sysdate-2);
commit;
INSERT INTO TMP_USER_ADDRESS
select * from martini_store.address@bayprod_saks_custom uad where uad_status_cd='ENB'
and exists (select 'x' from martini_store.user_account@bayprod_saks_custom where usa_uid is not null and usa_id=uad_usa_id)
...
from TMP_USER_ACCOUT tua
INNER JOIN TMP_USER_ADDRESS tuad  ON tua.USA_BILL_TO_UAD_ID= tuad.uad_id
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_str_val gndr from  tmp_object_attr_cust obg ) obg
            ON obg.oba_obj_id=tua.usa_id
            AND  obg.oba_atr_id=6193548999262730
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_dt_val brth_dt from  tmp_object_attr_cust obg ) obdt
            ON obdt.oba_obj_id=tua.usa_id
            AND obdt.oba_atr_id=6193548999262729
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_str_val rwrd from  tmp_object_attr_cust obg ) obrw
...
;
 ...
 BEGIN
  ...
  MERGE INTO BAY_DS.USER_ACCOUNT UA
  USING (
...
,tuad.UAD_STATUS_CD
from TMP_USER_ACCOUT tua
INNER JOIN TMP_USER_ADDRESS tuad  ON tua.USA_SHIP_TO_UAD_ID= tuad.uad_id
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_str_val gndr from  tmp_object_attr_cust obg ) obg
            ON obg.oba_obj_id=tua.usa_id
            AND  obg.oba_atr_id=6193548999262730
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_dt_val brth_dt from  tmp_object_attr_cust obg ) obdt
...
LEFT JOIN (select oba_atr_id,oba_obj_id,oba_str_val rwrd from  tmp_object_attr_cust obg ) obrw
            ...
            AND  obrw.oba_atr_id=6193548999262640
where tua.usa_uid is not null
AND NOT EXISTS (SELECT 'X' FROM BAY_DS.USER_ACCOUNT bua where bua.usa_email=tua.usa_uid)
--and rownum<10
;
  TYPE CUST_REC_TYPE IS
        TABLE OF C1%rowtype;
