...
--                 WHERE t1.order_line_key = ord(idx).order_line_key                    -- To Update Based on ORDER_LINE_KEY 
            ...
            THEN
                ...
                UPDATE &1.bi_sale t1 
                   SET 
                        t1.orderhdr = NVL(t1.orderhdr,ord(idx).orderhdr),
--                        t1.orderdet = NVL(t1.orderdet,ord(idx).orderdet),
--                        t1.orderseq = ord(idx).orderseq,              -- Not Needed if Existing Order is Update
                        t1.createfor = NVL(t1.createfor,ord(idx).createfor),
...
--                        t1.ordhdr_status = ord(idx).ordhdr_status,      -- Not Needed if Existing Order is Update
                        ...
                        t1.ordertype = NVL(t1.ordertype,ord(idx).ordertype),
                        t1.promo_id =  NVL(t1.promo_id,ord(idx).promo_id),
                        t1.freight_amt = ord(idx).freight_amt,
                        t1.giftorder_ind = NVL(t1.giftorder_ind,ord(idx).giftorder_ind),
                        t1.orig_price_amt = NVL(t1.orig_price_amt,ord(idx).orig_price_amt),
                        t1.offernprice_amt = NVL(t1.offernprice_amt,ord(idx).offernprice_amt),
                        t1.extend_price_amt = NVL(t1.extend_price_amt,ord(idx).extend_price_amt),
                        t1.qtyordered = ord(idx).qtyordered,
                        t1.taxrate_amt = NVL(t1.taxrate_amt,ord(idx).taxrate_amt),
                        t1.discount_amt = ord(idx).discount_amt,
                        t1.bm_skuid = ord(idx).bm_skuid,
                        t1.bill_to_address_id = NVL(t1.bill_to_address_id,ord(idx).bill_to_address_id),
                        t1.sourcecodedetail = ord(idx).sourcecodedetail,
                        t1.saksfirst_ind = NVL(t1.saksfirst_ind,ord(idx).saksfirst_ind),
                        t1.saksifirst_freeship_ind = NVL(t1.saksifirst_freeship_ind,ord(idx).saksifirst_freeship_ind),
                        t1.refer_id = ord(idx).refer_id,
                        t1.employee_ind = NVL(t1.employee_ind,ord(idx).employee_ind),
                        t1.giftwrap_ind = NVL(t1.giftwrap_ind,ord(idx).giftwrap_ind),
                        t1.giftwrap_msg_ind = ord(idx).giftwrap_msg_ind,
                        t1.backorder_ind = NVL(t1.backorder_ind,ord(idx).backorder_ind),
                        ...
                        t1.fill_loc = NVL(t1.fill_loc,ord(idx).fillloc),
                        ...
                        t1.ship_first_name = NVL(t1.ship_first_name,ord(idx).ship_first_name),
                        t1.ship_middle_name = NVL(t1.ship_middle_name,ord(idx).ship_middle_name),
                        t1.ship_last_name = NVL(t1.ship_last_name,ord(idx).ship_last_name),
                        t1.ship_addr1 = NVL(t1.ship_addr1,ord(idx).ship_addr1),
                        t1.ship_addr2 = NVL(t1.ship_addr2,ord(idx).ship_addr2),
                        t1.ship_addr3 = NVL(t1.ship_addr3,ord(idx).ship_addr3),
                        t1.ship_city = NVL(t1.ship_city,ord(idx).ship_city),
                        t1.ship_state = NVL(t1.ship_state,ord(idx).ship_state),
                        t1.ship_zipcode = NVL(t1.ship_zipcode,ord(idx).ship_zipcode),
                        t1.ship_country = NVL(t1.ship_country,ord(idx).ship_country),
                        t1.international_ind = NVL(t1.international_ind,ord(idx).international_ind),
                        t1.sub_total = NVL(t1.sub_total,ord(idx).sub_total),
                        t1.line_tax = NVL(t1.line_tax,ord(idx).line_tax),
                        t1.total_shipping = NVL(t1.total_shipping,ord(idx).total_shipping),
                        t1.gift_wrap_fee = NVL(t1.gift_wrap_fee,ord(idx).gift_wrap_fee),
                        t1.affiliate_id = NVL(t1.affiliate_id,ord(idx).affiliate_id),
...
--                        t1.product_id = ord(idx).product_id,
                        t1.sku = NVL(t1.sku,ord(idx).sku),
--                        t1.item = ord(idx).item,
                        t1.canceldate = ord(idx).canceldate,
                        t1.fullfilllocation = NVL(t1.fullfilllocation,ord(idx).fullfilllocation),
                        t1.more_number = NVL(t1.more_number,ord(idx).order_source),
--                        t1.csr_id = ord(idx).more_number,
--                        t1.order_source = ord(idx).csr_id,
--                        t1.customer_display_price = ord(idx).customer_display_price,
                        t1.srind = NVL(t1.srind,ord(idx).sr_ind),
                        t1.tracking_number = NVL(t1.tracking_number,ord(idx).tracking_number),
