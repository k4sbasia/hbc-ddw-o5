...
EXEC DBMS_OUTPUT.PUT_LINE ('o5_product_feed_dw.sql started at '|| to_char(sysdate , 'MM/DD/YYYY HH:MI:SS AM'));
...
--SELECT * FROM SAKS_CUSTOM.O5_SR_PRODUCTS_BM@PRDO5_SAKSCUSTOM;
--SELECT * FROM SAKS_CUSTOM.O5_SR_PRODUCTS_BM@SLOT31_SAKSCUSTOM; --for SLOT31 testing
...
EXEC DBMS_OUTPUT.PUT_LINE ('Preparing O5.O5_SR_PRODUCTS completed at '||to_char(sysdate , 'MM/DD/YYYY HH:MI:SS AM'));


EXEC DBMS_OUTPUT.PUT_LINE ('Update O5.BI_PRODUCT_INFO with shoprunner elligible started at '||to_char(sysdate , 'MM/DD/YYYY HH:MI:SS AM'));

UPDATE O5.BI_PRODUCT_INFO
SET SHOPRUNNER_ELIGIBLE = 1
WHERE PRODUCT_CODE     IN
  (SELECT PRODUCT_CODE FROM O5.O5_SR_PRODUCTS
  );
