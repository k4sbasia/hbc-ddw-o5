...
--- Set the language_id to french for Canadian store 327 
--- and for Canadian Store 336 (added on 08/08/2017)
update o5.edb_stage_sub 
set language_id = 'French'
where  batch_id = &1 
and skip = '0' 
and str_code in (select store_no from RFS.RF_STORE@saksrfs_prd 
				where chain = 7 and currency_code = 'CAD' and store_no in ('327','336')
				);
...
exec dbms_output.put_line('mv refresh');

exec DBMS_MVIEW.refresh('SDDW.mv_o5_email_address', 'C', ATOMIC_REFRESH => false);
show errors;
exec DBMS_MVIEW.refresh('SDDW.mv_o5_email_detail_trans', 'C');
show errors;
exit;
