...
UPDATE O5.email_address
   SET cheetah_extract_dt_delta = SYSDATE
 WHERE sys_entry_dt > (select curr_extract_time from o5.edb_sub_status) 
   AND opt_in = 1
   AND valid_ind = 1
   AND add_by_source_id <> '9104'
   AND email_address LIKE '%@%'
   AND email_address LIKE '%.%'
   and cheetah_extract_dt is null
   and cheetah_extract_dt_delta is null 
   ;
commit;

UPDATE O5.email_address
   SET cheetah_extract_dt_delta = SYSDATE
 WHERE sys_entry_dt > (select last_extract_time from o5.edb_sub_status)
   AND opt_in = 1
   AND valid_ind = 1
   AND add_by_source_id <> '9104'
   AND email_address LIKE '%@%'
