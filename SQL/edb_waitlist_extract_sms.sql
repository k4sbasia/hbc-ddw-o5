...
TRUNCATE TABLE &1.EDB_WAITLIST_EXTRACT_SMS_WRK;

INSERT INTO &1.EDB_WAITLIST_EXTRACT_SMS_WRK
            (
			 SKU_CODE_LOWER,
...
delete  from &1.EDB_WAITLIST_EXTRACT_SMS_WRK a
 where rowid>
 ( select min(rowid) from &1.EDB_WAITLIST_EXTRACT_SMS_WRK  b where
 a.phone_number=b.phone_number and a.sku_code_lower=b.sku_code_lower);

 commit;

MERGE INTO &1.WAITLIST trg
     USING (SELECT distinct phone_number, sku_code_lower  FROM  &1.EDB_WAITLIST_EXTRACT_SMS_WRK) src
...
INSERT INTO &1.EDB_WAITLIST_SMS_HIS
(
   SKU_CODE_LOWER,
   SKU_PRICE_WAITLISTED,
   ITEM_URL,
...
SELECT  distinct
   ...
   SYSDATE,
   PHONE_NUMBER
FROM &1.EDB_WAITLIST_EXTRACT_SMS_WRK where phone_number IS NOT NULL;
