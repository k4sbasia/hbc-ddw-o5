...
UPDATE o5.inventory_STORES_v1 set PROCESSED='P' WHERE PROCESSED='C';
...
;
 ...
 BEGIN
  ...
  WHEN MATCHED THEN
    ...
    IN_STORE_UPDATE_DATE=cp.instockdate,
    BATCH_ID=cp.batch_id,
    MERGE_BATCH_ID=v_prcs_batch
  WHEN NOT MATCHED THEN
    INSERT (SKN_NO,STORE_ID,IN_STORE_QTY,IN_STORE_UPDATE_DATE,ADD_DT,BATCH_ID,MERGE_BATCH_ID
            )
    VALUES (CP.ITEMID,CP.STOREID,CP.ONHAND,CP.instockdate,SYSDATE,cp.batch_id,v_prcs_batch
       );
       COMMIT;
...
WHEN MATCHED THEN
UPDATE SET INV.IN_STORE_QTY=SS.IN_STORE_QTY , INV.IN_STORE_UPDATE_DATE=SS.IN_STORE_UPDATE_DATE
WHEN NOT MATCHED THEN
