...
-----------------------------------------------------------------------------SFCC_PROD_SKU_DYN_FLAGS update/insert------------------------------------------------------------------------
	...
	CURSOR existing IS 
   		...
   		INNER JOIN SFCC_PROD_SKU_DYN_FLAGS sku ON pr.SKN_NO = sku.SKN 
   		;
   ...
   CURSOR not_existing IS
   		SELECT pr.ITEM_ID AS PRODUCT_ID, pr.SKN_NO as SKN, skunx.ISCLEARANCE AS EXISTING_ISCLEARANCE, skunx.IsFinalSale AS EXISTING_IsFinalSale,
   			...
   			ELSE 'false' END AS ACTUAL_IsFinalSale
   		FROM EDATA_EXCHANGE.O5_SD_PRICE pr
   		LEFT JOIN SFCC_PROD_SKU_DYN_FLAGS skunx ON pr.SKN_NO = skunx.SKN
   ...
   BEGIN 
		...
		SELECT max(PIM_CHG_DT) INTO v_LAST_CHG_DT FROM SFCC_PROD_SKU_DYN_FLAGS;
		...
		LOOP 
			...
			update SFCC_PROD_SKU_DYN_FLAGS
		...
		LOOP 
			...
			INSERT INTO SFCC_PROD_SKU_DYN_FLAGS(PRODUCT_ID, SKN, ISSALE, ISCLEARANCE, DYN_FLAG_CHG_DT, IN_STOCK, IN_STOCK_CHG_DT, STATUS, COLOR, SIZ, UPC, PIM_CHG_DT, IsFinalSale)
